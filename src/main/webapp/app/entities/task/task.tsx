...
export class Task extends React.Component<ITaskProps> {
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="task-heading">
          <Translate contentKey="cvServiceApp.task.home.title">Tasks</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="cvServiceApp.task.home.createLabel">Create new Task</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {taskList.map((task, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{task.label}</td>
                  <td>{task.experience ? <Link to={`experience/${task.experience.id}`}>{task.experience.id}</Link> : ''}</td>
